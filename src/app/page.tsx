...
export default function Home() {
  return (
    <main className="mx-auto min-h-screen w-full max-w-6xl bg-background px-5 py-5 md:px-8 md:py-7">
      ...
      <section className="grid gap-8 md:grid-cols-[1.05fr_0.95fr] md:items-center">
        <div className="space-y-4 md:space-y-5">
          ...
          <h1 className="max-w-2xl text-[2.7rem] leading-[0.98] tracking-[-0.055em] md:text-[4.05rem]">
            Fill open shifts faster while giving clinicians more{" "}
          ...
          <p className="max-w-xl text-[0.98rem] leading-7 text-muted-foreground md:text-[1.02rem]">
            ShiftLink connects facilities needing dependable coverage with qualified professionals
            ready for flexible, high-value shifts.
          </p>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/signup?type=professional"
              className="rounded-full bg-primary px-5 py-2.5 text-[0.84rem] font-medium text-primary-foreground transition hover:opacity-90"
            >
              Join as Professional
            </Link>
            <Link
              href="/signup?type=facility"
              className="rounded-full border border-border px-5 py-2.5 text-[0.84rem] font-medium text-foreground transition hover:bg-secondary"
            >
              Post a Shift
            </Link>
          </div>
        ...
        <div className="space-y-3 md:pt-6">
          <article className="rounded-2xl border border-border bg-card/80 p-4 shadow-[0_18px_60px_rgba(35,35,25,0.04)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-2 text-[1.35rem] leading-tight tracking-[-0.035em]">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <article className="rounded-2xl border border-border bg-card/70 p-4">
              ...
              <p className="mt-1 text-[1.05rem] font-semibold tracking-[-0.025em] text-primary">
                Claimed successfully
              </p>
              <p className="mt-2 text-[0.75rem] leading-5 text-muted-foreground">
                Assigned to RN · 11 mins ago
              </p>
            </article>
          </div>
        </div>
      ...
      <section id="shift-board" className="mt-10 bg-background px-0 pb-8">
        <p className="text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-muted-foreground">
          Shift Board Preview
        </p>
        <h2 className="mt-2 max-w-2xl text-[1.85rem] leading-tight tracking-[-0.045em] md:text-[2.2rem]">
          Available shifts, clearly presented.
        </h2>
        <p className="mt-2 max-w-3xl text-[0.95rem] leading-7 text-muted-foreground">
          Professionals can browse open shifts by role, date, time, facility, and rate before
          claiming.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {previewShifts.map((shift) => (
            <article
              key={`${shift.role}-${shift.facility}`}
...
function UserPathCard({
  ...
  return (
    <article
      id={id}
      className="flex h-full min-h-[255px] flex-col rounded-2xl border border-border bg-card/75 p-5 md:p-6"
    >
      <p className="text-[0.68rem] font-semibold uppercase tracking-[0.26em] text-muted-foreground">
        {label}
      </p>
      <h3 className="mt-2 max-w-[26rem] text-[1.75rem] leading-[1.03] tracking-[-0.05em] md:text-[2.05rem]">
        {title}
      </h3>
      <p className="mt-3 max-w-[30rem] text-[0.92rem] leading-6 text-muted-foreground">
        {description}
      </p>
